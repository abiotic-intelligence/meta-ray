...
import io.metaray.jvm.transport.TlsControlServer;
...
public final class LanMetaRayEmitter implements MetaRayEmitter, AutoCloseable {
  ...
  public LanMetaRayEmitter(EmitterConfig info) {
    this(info, new JacksonJsonCodec(), new JacksonCborCodec(), new JmDnsAdvertiser(), null, null);
  }

  public LanMetaRayEmitter(EmitterConfig info, JacksonJsonCodec codec, JmDnsAdvertiser advertiser) {
    this(info, codec, advertiser, null);
  }

  public LanMetaRayEmitter(
      EmitterConfig info,
      JacksonJsonCodec codec,
      JmDnsAdvertiser advertiser,
      MulticastTransport multicastTransport
  ) {
    this(info, codec, new JacksonCborCodec(), advertiser, null, multicastTransport);
  }
  ...
  public LanMetaRayEmitter(
    ...
    this.tlsControlServer = new TlsControlServer(info.controlPort, factory,
        io.metaray.jvm.internal.JvmExecutors.newCachedIoExecutor("metaray-jvm-tls-control-server-io"));
    ...
    } else if (info.multicastPort != null) {
  ...
  public CompletableFuture<Void> startAdvertisingAsync() {
    try {
      ensureNotShutdown();
      if (advertiser != null) advertiser.start(info);
